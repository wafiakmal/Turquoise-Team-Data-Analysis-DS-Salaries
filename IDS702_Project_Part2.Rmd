...
2) For the second question, the response variable (remote ratio) is a categorical variable with 3 levels: 0, 50% and 100%, which represents an in-person job, a hybrid job, and a fully remote job, respectively. Thus the multiple linear regression model from first question is not applicable here. In this case, we can use multinomial logistic regression model. The log odds of remote ration will be calculated as a combination of all the predictor variables we are interestd in (such as the company size, percentage of supporting remote job, employment type). 
